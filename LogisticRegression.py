...
class LogisticRegression(object):
    ...
    def __init__(self,n_in,n_out,labels=None,Regularization=2,eta=0.0001):
        ...
        self.eta=eta;
        self.mask = np.ones(shape=(n_out, n_in));
    ...
    def initialize_parameters(self,n_in,n_out):
        ...
        size=(n_out, n_in);
        # self.W=numpy.asarray(np.random.RandomState(200).uniform(low,high,size),dtype=float);
    ...
    def predict(self,x): 
        a = self.W; 
        self.W = np.multiply(self.W,self.mask); #pruning in forward pass
    ...
    def negative_log_likelihood(self,params=None):
        ...
        self.W = np.multiply(self.W,self.mask); #jiang
...
##        self.validate(self.x,self.y)     
    ...
    def compute_gradients(self,out,y,x):
        ...
        if self.Regularization==1:
           ...
           #dd
        # jiang
        pruned_W = np.multiply(W,self.mask);
        #res=np.vstack((W.T,out.flatten()))
        res=np.vstack((pruned_W.T,out.flatten()))
        return res;
    ...
    def gradients(self,params=None):
        ...
        x,y=self.args;
        self.update_params(params);
        #self.W = np.multiply(self.W,self.mask); #jiang this is unnecessary bcs it's been done in forward pass prunning
    ...
    """ function to set the pparamsarams """        
    ...
    def prune_layer(self,drop_thres=0):
        ...
        """
        print "-- Output Layer Before: %d non-zeros" % np.sum(self.mask);
        if drop_thres != 0:
            self.mask = np.abs(self.W) > drop_thres;
        print "-- Output Layer After: %d non-zeros" % np.sum(self.mask);

    def refresh_params(self):
        param1=self.params.reshape(-1,self.nparams);        
        param1[:,0:self.nparams-1]=self.W;
        param1[:,self.nparams-1]=self.b.reshape(-1,self.n_classes);   
